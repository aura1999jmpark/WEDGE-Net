...
# Color Distortion Intensities (0.5 to 3.0)
INTENSITIES = [0.5, 1.0, 2.0, 3.0] 

IMAGENET_MEAN = [0.485, 0.456, 0.406]
IMAGENET_STD  = [0.229, 0.224, 0.225]

# [Config] Ratio Parsing (Default to 1% / 0.01)
raw_ratio = str(getattr(config, 'SAMPLING_RATIO', '0.01')).lower()

if raw_ratio == 'all':
    print("\n" + "-"*60)
    print(" [Info] Configured SAMPLING_RATIO = 'all'.")
    print(" To ensure concise visualization, we focus on the Main Proposed Method (1%).")
...
def find_best_model_path(base_dir, category, target_str=None):
    """
    Scans 'base_dir' to find a .pt file that matches the 'category'.
    ...
    # If explicit target string provided (e.g., '10pct'), prioritize it
    if target_str:
        priority = [f for f in candidates if target_str in f]
        if priority: return os.path.join(base_dir, priority[0])
    
    # Default: Return shortest filename (usually cleanest) or first found
    candidates.sort(key=len)
    return os.path.join(base_dir, candidates[0])

# ==============================================================================
# 3. Utility Classes (UnNormalize Fixed)
# ==============================================================================
class UnNormalize(object):
    ...
    def __call__(self, tensor):
        ...
        t_copy = tensor.clone()
        # Broadcasting fix to prevent red tint
...
def apply_color_jitter(image_tensor, factor):
    ...
    if factor == 0: return image_tensor.clone()
    jittered = image_tensor.clone()
    
    jittered = transforms.functional.adjust_brightness(jittered, 1.0 + (factor * 0.3)) 
    jittered = transforms.functional.adjust_contrast(jittered, 1.0 + (factor * 0.5))
    jittered = transforms.functional.adjust_saturation(jittered, 1.0 + (factor * 0.5))
    
    hue_factor = min(0.1 * factor, 0.5)
    jittered = transforms.functional.adjust_hue(jittered, hue_factor)
...
def run_color_experiment():
    print("\n" + "="*60)
    print(" 🎨 Starting Color Robustness Experiment")
    print("="*60)
    os.makedirs(BASE_DIR, exist_ok=True)

    # [Logic] Validate Category Selection (Force Single)
    ...
    for category in target_categories:
        ...
        # Prioritize 10% model for fair comparison
        ...
        # Optimization: Downsample large memory banks
        ...
        for i in range(len(dataset)):
            ...
            if (label.item() if hasattr(label, 'item') else label) == 0:
                ...
                print(f"   [Data] Selected Normal Image: {os.path.basename(path[0])}")
                break
        ...
        for i, intensity in enumerate(INTENSITIES):
            ...
            ax.set_ylabel(f"Level {intensity}", fontsize=16, fontweight='bold')
        ...
        if path_pc and "10pct" in os.path.basename(path_pc):
            pc_label = "10%"
        elif path_pc:
            pc_label = "100%"
        else:
        ...
        cols = ['Original', 'Color Shifted', f'PatchCore ({pc_label})', f'WEDGE-Net ({wd_label})']
        for ax, col in zip(axes[0], cols):
            ax.set_title(col, fontsize=18, fontweight='bold', pad=10)

        save_path = os.path.join(BASE_DIR, f"Figure_Color_Robustness_{category}_{ratio_suffix}.png")
